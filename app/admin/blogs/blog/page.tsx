    ...
    const fetchPost = async () => {
        try {
            const { data, error } = await supabase
                .from('posts')
                .select('*, categories(title), subcategories(title), profiles(full_name)')
                .eq('id', id)
                .single();
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-12">
                        <div className="max-w-4xl">
                            <div className="flex flex-wrap items-center gap-3 mb-6">
                                ...
                                {post.subcategories && (
                                    <span className="px-4 py-1.5 rounded-full bg-white/20 backdrop-blur-md text-white text-xs font-black uppercase tracking-widest border border-white/20">
