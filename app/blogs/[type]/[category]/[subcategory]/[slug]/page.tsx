...
export default async function PostPage({ params }: { params: Promise<{ type: string, category: string, subcategory: string, slug: string }> }) {
    ...
    // Optional: Strict validation to ensure the post_type in DB matches the URL segment
    const dbType = post.post_type === 'blog' ? 'post' : post.post_type; // user wants 'post' for 'blog'
    if (type !== dbType) {
        // You could redirect or 404 here for extreme SEO strictness
        // return redirect(`/blogs/${dbType}/${category}/${subcategory}/${slug}`);
    }
